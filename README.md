...
- **老师后台 `/admin`**：审核 / 发布 / 下线学生提交；也可直接新增同学；「一键准备素材」按顺序生成卡通形象、
  说话循环视频、每部分的讲解文字与语音（已有的自动跳过）；一键复制学生提交入口的链接和二维码。
...
- **预生成讲解 + 语音**：后台可提前让 AI 写好每部分的讲解并合成好语音（A2E），访客选中时零等待直接播放；
  追问永远实时生成，文字边流式输出边逐句朗读，第一句话几秒内开口。
...
| `AI_MODEL_LIVE` | 可选但**建议设置**为一个更快的模型。现场实时回答（追问、未预生成的部分）用它；分段和预生成仍用 `AI_MODEL`。 |
...
| `TRUST_PROXY` | 可选。部署在自己的反向代理（Nginx 等）后面时设为 `1`，限流才按真实访客 IP 计数；Vercel 上自动识别。 |
...
- 后台：单一密码（恒定时间比较），登录 cookie 带签名和 12 小时有效期；未设置密码时拒绝所有登录。
- 学生提交：公开入口，提交后**必须审核**才上线；每条记录有独立的随机修改 token，写操作只认请求头里的 token；
  预览链接用的是由它派生的只读 token，发给别人看也改不了内容。
- 上传图片按文件头校验，只接受 JPG / PNG / WebP（拒绝 SVG 等可执行内容），上限 4 MB；
  浏览器会先把大图缩到 1600px 再上传，手机原图也很快。
- Blob 上的记录（含修改 token）是**私有**对象，只有服务器能读；照片 / 卡通 / 视频 / 语音是带随机后缀的公开对象。
- 公开的 AI / TTS / 提交接口有基于 IP 的限流（按实例计数，阈值对同一校园网出口留了余量）：
  访客问答 150 次/分钟，实时（未缓存）AI 回答 600 次/小时作为花费刹车，学生 AI 分段 200 次/10 分钟，登录 10 次/分钟。
  Vercel 上自动按 `x-forwarded-for` 取真实 IP，自建反代需设 `TRUST_PROXY=1`。
- 响应头带 `X-Content-Type-Options: nosniff`、`X-Frame-Options: SAMEORIGIN`、Referrer-Policy、Permissions-Policy。
...
3. **持久化存储（Vercel Blob）**：Vercel 函数文件系统是只读且临时的，所以线上自动改用 Vercel Blob。
   ...
   - 连接后 **Redeploy** 一次即生效。
4. 数据布局（Blob）：每位同学的记录是**私有**对象 `people/<id>/<版本>.json`（每次保存写一个新版本，只保留最近两个），
   slug 索引 `slugs/<slug>` 一次性写入保证不重名，多人同时提交互不覆盖；照片 / 卡通 / 视频 / 语音是公开对象。
   旧布局（单文件 `data/people.json`，或早期公开的 `people/<id>.json` + `slugs/<slug>.txt`）会在首次访问时自动迁移并删掉旧对象。
5. 登录后打开 `/api/admin/health` 可查看线上实际生效的配置（只显示布尔值，不显示密钥）。
6. Hobby 套餐的函数上限是 60 秒：「预生成讲解与语音」是逐部分落盘的，超时只会少几段，再点一次会补齐剩下的；
   讲稿很长的同学建议用「一键准备素材」按人逐个跑。

> 也可以部署到一台**常驻服务器**（`npm run build && npm run start`），文件系统存储即可正常工作
> （每人一个文件在 `data/people/`，上传的媒体在 `data/uploads/`）。
...
- 克隆每位同学的真实声音（需录制音样）。
- 后台数据看板：每位同学被问最多的问题等。
- 用 Redis / KV 做跨实例的精确限流（现在每个实例各自计数）。
- 修改 token 只存哈希（现在明文存在私有记录里，好处是老师能在后台重发学生链接）。
